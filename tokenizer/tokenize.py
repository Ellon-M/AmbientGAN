...
class NoteTokenizer:
    ...
    def lspToPy(self, path_to_notes):
        ...
        with open(path_to_notes, "rb") as f:
            ...
            for x in re.split(r'[\n]', str_note_file):
                x = replace_all({"((": "",  "))": ",",  "(": "",  ")": "",  "'": "",  '"': '', "b": "", }, x) # removes and replaces unwanted lisp strings if any
                x = str(x) 
                x = x.split("\\r") # makes each note sequence an array item
                self.notes.append(x) 
    ...
    def generate(self, notes, model_path, sample):
        """ Generate note samples from the network.
            ...
            Parameters:
                ...
                model_path: string
                sample: int
                    a single sample of the model output from the generated batch(128 samples) to generate notes from
        ...
        pred_notes_unnorm = [((x * n_vocab/2) + n_vocab/2) for x in generated[sample]]
